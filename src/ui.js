  ...
  const addToDo = function (e) {
    ...
    if (!title || !description || !date) {
      ...
      if (e.target.getAttribute("data-todo") === null) {
        // new toDo
        myProjects[`${e.target.getAttribute("data-position")}`].toDoArray.push(
          newToDo
        );
      } else {
        // editing toDo
        myProjects[e.target.getAttribute("data-position")].toDoArray.splice(
...
const interfaceElements = (function (S) {
  ...
  const minimize = function (e) {
    ...
    for (
      let i = 0;
      i < myProjects[e.target.getAttribute("data-position")].toDoArray.length;
      i++
    ) {
  ...
  const toggle = function (e) {
    ...
    if (
      card.getElementsByTagName("h4")[0].style.display === "" ||
...
export { addElements, deleteElements, editElements, interfaceElements };
